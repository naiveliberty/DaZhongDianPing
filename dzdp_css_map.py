...
class DaZhongDianPing():
    ...
    def get_font_map(self):
        ...
        review_result = re.findall('<textPath xlink:href="#(\d+)" textLength=".*?">(.*?)</textPath>', self.review_svg, re.S)
        tell_result = re.search('<text x="(.*?)" y="36">(.*?)</text>', self.tell_svg, re.S)
        ...
        self.address_font_map = self.class_to_font(address_class_list, address_svg_y_list, address_words_dc, address_prefix)
        self.review_font_map = self.class_to_font(review_class_list, review_svg_y_list, review_words_dc, review_prefix)
        self.tell_font_map = self.class_to_tell_num(tell_class_list, tell_x_list, tell_words_str, tell_prefix)
    ...
    def class_to_font(self, class_list, y_list, words_dc, prefix):
        tmp_dc = dict()
        # 核心算法，将 css 转换为对应的字符
        for i in class_list:
            x_id = None
    ...
    def class_to_tell_num(self, class_list, x_list, words_str, prefix):
        tmp_dc = dict()
        for i in class_list:
            x_index = None
